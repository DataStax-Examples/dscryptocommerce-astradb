...
const ProductDetails: NextPage<InferGetServerSidePropsType<typeof getServerSideProps>> = ({
  ...
  const processNft = async (
    ...
    if (operation === "listNft") {
      ...
      const session_res = await trackSession(dateTime, account as string, productId, "List NFT Clicked", ASTRA_SESSION_URL);
      if (listNftTx) {
    ...
    if (operation === "bidNft") {
      ...
      const session_res = await trackSession(dateTime, account as string, productId, "Bid NFT Clicked", ASTRA_SESSION_URL);
      if (bidNftTx) {
    ...
    if (operation === "shipNft") {
      ...
      const session_res = await trackSession(dateTime, account as string, productId, "Ship NFT Clicked", ASTRA_SESSION_URL);
      if (shipNftTx) {
    ...
    if (operation === "receiveNft") {
      ...
      const session_res = await trackSession(dateTime, account as string, productId, "Receive NFT Clicked", ASTRA_SESSION_URL);
      if (receiveNftTx) {
        const astra_url = `${process.env.NEXT_PUBLIC_ASTRA_DB_URL}/catalog/${productId}`
        const res = await axios.patch(astra_url, {
